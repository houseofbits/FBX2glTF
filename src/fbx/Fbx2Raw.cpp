...
static void ReadMesh(
 ...
 for (int polygonIndex = 0; polygonIndex < pMesh->GetPolygonCount(); polygonIndex++) {
   ...
   for (int vertexIndex = 0; vertexIndex < 3; vertexIndex++, polygonVertexIndex++) {
     ...
     vertex.position[1] = (float)fbxPosition[yIndex] * scaleFactor;
     vertex.position[2] = (float)fbxPosition[zIndex] * scaleFactor;
     vertex.normal[0] = (float)fbxNormal[0];
     vertex.normal[1] = (float)fbxNormal[yIndex];
     vertex.normal[2] = (float)fbxNormal[zIndex];
     vertex.tangent[0] = (float)fbxTangent[0];
     vertex.tangent[1] = (float)fbxTangent[yIndex];
     vertex.tangent[2] = (float)fbxTangent[zIndex];
     vertex.tangent[3] = (float)fbxTangent[3];
     vertex.binormal[0] = (float)fbxBinormal[0];
     vertex.binormal[1] = (float)fbxBinormal[yIndex];
     vertex.binormal[2] = (float)fbxBinormal[zIndex];
...
bool LoadFBXFile(
 ...
 if (options.ignoreUnitConversion) {
   scaleFactor = 1.0;
 }
